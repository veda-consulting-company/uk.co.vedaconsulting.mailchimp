 ...
 * extension.
 */
class CRM_Mailchimp_ExtensionUtil {
  const SHORT_NAME = 'mailchimp';
  const LONG_NAME = 'uk.co.vedaconsulting.mailchimp';
  const CLASS_PREFIX = 'CRM_Mailchimp';
 ...
 */
function _mailchimp_civix_civicrm_postInstall() {
  _mailchimp_civix_civicrm_config();
  if ($upgrader = _mailchimp_civix_upgrader()) {
 ...
 */
function _mailchimp_civix_civicrm_themes(&$themes) {
  $files = _mailchimp_civix_glob(__DIR__ . '/*.theme.php');
  foreach ($files as $file) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _mailchimp_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _mailchimp_civix_civicrm_entityTypes(&$entityTypes) {
